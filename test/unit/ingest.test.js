...
const { ingestTrack, ingestBatch } = await import("../../src/services/ingestion/ingest.js");
...
describe("ingestTrack – REQ-001: saves a new track", () => {
  ...
  it("returns status 'created' with the created track", async () => {
...
describe("ingestTrack – REQ-002: artist tag fallback", () => {
  ...
  it("chooses best source, does not merge track and artist tags", async () => {
    const client = {
      getTrackInfo: async () => defaultTrackInfo(),
      getTrackTopTags: async () => [{ name: "rock", count: 100 }],
      getArtistTopTags: async () => [
    ...
    const hasTrackTag = tags.some((t) => t.toLowerCase() === "rock");
    assert.equal(hasTrackTag, false, "track tags should NOT appear when artist source is chosen");
    assert.ok(tags.includes("pop"), "artist tags should be used as best source");
  });
});
...
describe("ingestTrack – REQ-005: graceful error handling", () => {
  it("returns status 'failed' when getTrackInfo throws", async () => {
    stubbedFindOne = async () => null;
    ...
    assert.equal(result.error, "Last.fm unavailable");
  });

  it("returns status 'failed' when getTrackTopTags throws", async () => {
    stubbedFindOne = async () => null;
  ...
  it("returns status 'failed' when Track.create throws", async () => {
    stubbedFindOne = async () => null;
    stubbedCreate = async () => {
...
describe("ingestBatch – REQ-004: processes multiple tracks sequentially", () => {
  ...
  it("returns { results, summary } with one result per input track", async () => {
    const client = makeClient();
    const tracks = [
    ...
    const batch = await ingestBatch(client, tracks);

    assert.ok(batch.results, "batch should have results array");
    assert.ok(batch.summary, "batch should have summary object");
    assert.equal(batch.results.length, 2);
    assert.equal(batch.summary.created, 2);
  });
  ...
  it("each result contains artist and title", async () => {
    ...
    assert.equal(batch.results[0].artist, "Queen");
    assert.equal(batch.results[0].title, "Bohemian Rhapsody");
    assert.equal(batch.results[1].artist, "Radiohead");
    assert.equal(batch.results[1].title, "Creep");
  });
  ...
  it("does not throw when one track fails", async () => {
    ...
    assert.equal(batch.results[1].status, "created");
  ...
  it("passes force option to each ingestTrack call", async () => {
    ...
    const batch = await ingestBatch(client, tracks, { force: true });
    assert.equal(batch.results[0].status, "updated");
  });

  it("processes an empty array without error", async () => {
    const client = makeClient();
    const batch = await ingestBatch(client, []);
    assert.deepEqual(batch.results, []);
    assert.deepEqual(batch.summary, { created: 0, skipped: 0, failed: 0 });
  });
});
