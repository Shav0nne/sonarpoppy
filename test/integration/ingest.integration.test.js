...
describe("ingestTrack – artist tag fallback (REQ-002)", () => {
  it("falls back to artist tags when track tags are sparse, resulting in a non-zero vector", async () => {
    ...
    // Best source chosen (artist has more mapped genres), not merged
    const hasTrackTag = doc.lastfmTags.includes("rock");
    assert.equal(
      hasTrackTag,
      false,
      "track tags should NOT be present when artist source is chosen",
    );
    assert.ok(
      doc.lastfmTags.includes("pop") || doc.lastfmTags.includes("electronic"),
      "artist tags should be used as best source",
    );
  });
});
...
describe("ingestBatch – processes multiple tracks (REQ-004)", () => {
  it("saves all tracks to the database", async () => {
    ...
    assert.ok(batch.results.every((r) => r.status === "created" || r.status === "failed"));
  ...
  it("skips already-ingested tracks in the same batch", async () => {
    ...
    assert.ok(batch.results.every((r) => r.status === "skipped" || r.status === "created"));
    const countSkipped = batch.results.filter((r) => r.status === "skipped").length;
  ...
  it("force updates existing tracks in batch", async () => {
    ...
    const batch = await ingestBatch(client, [{ artist: "Queen", title: "Bohemian Rhapsody" }], {
      force: true,
    });
